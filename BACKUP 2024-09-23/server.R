server <- function(input, output, session) {
  ...
  output$data <- renderDT({
    ...
    datatable(TempDF, options = list(scrollX = TRUE))
  })
  ...
  observe({
    ...
    # Aktualisiere die Reihenfolge für group_order
    updateOrderInput(session,
                     inputId = "group_order",
                     items = unique(DataTable()[[input$groupnameis]]),
                     item_class = 'info')
  })
  ...
  BoxplotInput <- reactive({
    req(input$selected_groups,
        input$group_order,
        SelectionGroupBoxplot())  # Sicherstellen, dass die Tabelle vorhanden ist
  ...
  BarplotInput <- reactive({
    req(input$selected_groups,
        input$group_order,
        SelectionGroupBarplot())  # Sicherstellen, dass die Tabelle vorhanden ist
